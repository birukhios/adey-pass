...
export function ScannerPanel({ gates }: { gates: Array<{ id: string; name: string }> }) {
  ...
  return (
    <div className="grid gap-4 rounded-[1.75rem] border p-4 sm:p-5" style={{ borderColor: "color-mix(in oklab, white 12%, transparent)", background: "color-mix(in oklab, white 7%, transparent)" }}>
      <div>
        ...
        <div className="mt-4 grid gap-3 lg:grid-cols-[1fr_190px]">
          ...
          <button className="ap-button-primary min-h-12 gap-2 disabled:opacity-60 lg:col-span-2" disabled={!query.trim() || loading} onClick={() => { void lookupAndCheckin(); }} type="button">
      ...
      {result ? (
        <div className="rounded-3xl border p-5" style={{ borderColor: "color-mix(in oklab, white 12%, transparent)", background: "color-mix(in oklab, black 18%, transparent)" }}>
          <p className="text-xs font-black uppercase tracking-[0.16em] text-white/45">Manual result</p>
          <h2 className="mt-2 text-3xl font-black text-white">{message || "Result"}</h2>
          <dl className="mt-5 grid gap-4 text-sm text-white">
            <div><dt className="font-bold text-white/45">Guest</dt><dd className="font-black">{result.guest.fullName}</dd></div>
            <div><dt className="font-bold text-white/45">Event</dt><dd className="font-black">{result.event.name}</dd></div>
