  ...
  return (
    <>
      ...
      <div className="grid gap-5 md:grid-cols-2">
        <label className="grid gap-2 text-sm font-bold text-slate-700">Name<input className="ap-input" onChange={(event) => setForm((s) => ({ ...s, name: event.target.value }))} value={form.name} /></label>
        ...
        <label className="grid gap-2 text-sm font-bold text-slate-700">New password<input className="ap-input" onChange={(event) => setForm((s) => ({ ...s, newPassword: event.target.value }))} type="password" value={form.newPassword} /></label>
      </div>
      {message ? <div className="mt-4 rounded-lg bg-slate-100 p-3 text-sm font-bold text-slate-700">{message}</div> : null}
      <div className="mt-6"><button className="ap-button-primary disabled:opacity-60" disabled={loading} onClick={() => { void save(); }} type="button">{loading ? "Saving..." : "Save Profile"}</button></div>
    </>
  );
}
