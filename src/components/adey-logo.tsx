...
export function AdeyLogo({ compact = false, className, theme = "dark" }: { compact?: boolean; className?: string; theme?: "dark" | "light" }) {
  ...
  return (
    <div className={cn("flex items-center gap-3", className)}>
      <div className="relative grid size-12 shrink-0 place-items-center rounded-2xl bg-[var(--adey-yellow)] shadow-[0_0_0_6px_color-mix(in_oklab,var(--adey-yellow)_16%,transparent)]">
      ...
      {!compact && (
        <div>
          <div className={cn("text-xl font-black tracking-tight", titleClass)}>
            Stadium <span className="text-[var(--adey-yellow)]">Management</span>
          </div>
          <div className={cn("text-xs font-medium", subtitleClass)}>System</div>
        </div>
      )}
    </div>
  );
